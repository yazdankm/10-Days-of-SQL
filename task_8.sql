...
LEFT JOIN Trained_in ti ON  u.procedure = ti.treatment AND u.physician = ti.physician
...
JOIN Undergoes u ON u.physician = ph.employeeid
LEFT JOIN Trained_in ti ON  u.procedure = ti.treatment AND u.physician = ti.physician
JOIN Procedures pr ON u.procedure = pr.code
...
FROM Physician ph
JOIN Undergoes u ON u.physician = ph.employeeid
JOIN Trained_in ti ON  u.physician = ti.physician
WHERE ti.CertificationExpires < u.date;
...
SELECT ph.name, pr.name, u.date, pa.name, ti.CertificationExpires
FROM Physician ph
JOIN Undergoes u ON u.physician = ph.employeeid
JOIN Trained_in ti ON  u.physician = ti.physician
JOIN Procedures pr ON u.procedure = pr.code
...
SELECT pa.name, ph.name, n.name, a.start, a."End", a.examinationroom, pa.pcp, a.physician
FROM Appointment a
...
JOIN Block b ON b.floor = oc.blockfloor AND b.code = oc.blockcode
JOIN Room r ON b.floor = r.blockfloor AND b.code = r.blockcode
WHERE r.number = 123;
...
SELECT examinationroom, COUNT(examinationroom) AS n_appointments
FROM Appointment
...
WITH n_apps AS (
SELECT patient, COUNT(patient) AS n_apps
FROM Appointment 
GROUP BY patient
)
SELECT pa.name AS patient_name, ph.name AS physician_name
FROM Patient pa
JOIN Prescribes pr ON pa.ssn = pr.patient AND pa.pcp = pr.physician
JOIN Undergoes u ON pa.ssn = u.patient
JOIN Procedures p ON p.code = u.procedure
JOIN Nurse n ON n.employeeid = u.assistingnurse
JOIN n_apps ON n_apps.patient = pa.ssn
JOIN Physician ph ON ph.employeeid = pr.physician
JOIN Affiliated_with aw ON aw.physician = ph.employeeid
JOIN Department d ON d.departmentid = aw.department
WHERE p.cost > 5000 AND pr.physician != d.head AND n_apps.n_apps >= 2;
